...
@section('content')
    @include('layouts.navbars.auth.topnav', ['title' => 'Edit Pertanyaan'])
    ...
    <div class="container-fluid py-4">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form class="user" action="{{ route('pertanyaan.update', $pertanyaan->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row">
                        <div class="col-sm-3 mb-3">
                            ...
                            <select class="form-control @error('kode') is-invalid @enderror" id="kode" name="kode"
                                ...
                                <option value="1" {{ $pertanyaan->kode == 1 ? 'selected' : '' }}>Pengetahuan</option>
                                <option value="2" {{ $pertanyaan->kode == 2 ? 'selected' : '' }}>Perilaku</option>
                            </select>
                        </div>
                        <div class="col-sm-9 mb-3 mb-sm-0">
                            ...
                            <textarea class="form-control @error('soal') is-invalid @enderror" id="soal" name="soal"
                                placeholder="Masukan Pertanyaan Anda" required>{{ old('soal',$pertanyaan->soal ) }}</textarea>
                            @error('soal')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
