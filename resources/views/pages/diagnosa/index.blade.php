    ...
    @include('layouts.navbars.auth.topnav', ['title' => 'Diagnosis'])
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="deleteModalLabel">Hapus Diagnosis</h5>
        ...
        <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('importdiagnosa') }}" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p class="text-danger">ANDA AKAN MENGUBAH STATUS ACC SEMUA PEMERIKSAAN DIAGNOSA ATAS NAMA PASIEN INI</p>
                </div>
                <div class="modal-footer">
                ...
                <div class="col-12">
                    <div class="card mb-4">
                        <div class="card-header d-sm-flex align-items-center justify-content-between py-3">
                            ...
                            @can('adminmahasiswa')
                            <a href="{{route('diagnosa.create')}}" class="d-sm-inline-block btn btn-primary btn-sm shadow-sm">
                        ...
                        {{-- <div class="card-header d-sm-flex align-items-center">
                            @can('adminpembimbing')
                            <a href="{{route('exportdiagnosa')}}" class="d-sm-inline-block btn btn-primary btn-sm shadow-sm me-3">
                                ...
                                <table class="table align-items-center mb-0" id="dataTable">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            @can('admin')
                                                <td>{{ $diagnosa->user->username }}</td>
                                                <td>{{ ucwords(get_v('users', 'nimnip', $diagnosa->user->pembimbing, 'username')[0]->username ?? "") }}</td>
                                            @endcan
                                            @can('mahasiswa')
                                                <td>{{ ucwords(get_v('users', 'nimnip', $diagnosa->user->pembimbing, 'username')[0]->username ?? "") }}</td>
                                            @endcan
                                            @can('pembimbing')
                                                <td>{{ $diagnosa->user->username }}</td>
                                            @endcan
                                            <td>{{ $diagnosa->kartupasien->no_kartu }} | {{ $diagnosa->kartupasien->nama }}</td>
                                            
                                            <th>{{ $diagnosa->gigi }}</th>
                                            <th>{{ $diagnosa->masalah }}</th>
                                            <th>{{ $diagnosa->askepgilut }}</th>
                                            <th>{{ $diagnosa->penyebab }}</th>
                                            <th>{{ $diagnosa->gejala }}</th>
                                            
                                            <td style="display: none;">
                                                @can('mahasiswa')
                                                    <label
                                                        class="{{ $diagnosa->acc == 0 ? 'btn btn-danger' : 'btn btn-success' }}">{{ $diagnosa->acc == 0 ? 'Belum' : 'Sudah' }}</label>
                                                @endcan
                                                @can('adminpembimbing')
                                                    @if ($diagnosa->acc == 0)
                                                        <a href="javascript:;" data-toggle="modal"
                                                            onclick="handleACC({{ $diagnosa->id }})"
                                            ...
                                            <td>
                                                @can('mahasiswa')
                                                    <label class="{{ $diagnosa->acc == 0 ? 'btn btn-danger' : 'btn btn-success' }}">
                                                        {!! $diagnosa->acc == 0 ? '<i class="fas fa-times"></i>' : '<i class="fas fa-check"></i>' !!}
                                                    </label>
                                            ...
                                            <td>
                                                ...
                                                <a href ="{{route('diagnosa.show', $diagnosa->id)}}" title="show" class="btn btn-sm btn-icon-split btn-primary">
                                                    <span class="icon"><i class="fas fa-eye text-white" style="padding-top: 4px;"></i></span><span class="text">Lihat</span>
                                                </a>
                                                {{-- <a data-toggle="modal" id="smallButton" data-target="#smallModal"
                                                    data-attr="{{ route('anamridiagnosa.show', $project->id) }}" title="show">
                                                    <i class="fas fa-eye text-success  fa-lg"></i>
                                                </a> --}}
                                                @can('adminmahasiswa')
                                                <a href ="{{route('diagnosa.edit', $diagnosa->id)}}" title="Edit" class="btn btn-sm btn-icon-split btn-warning">
                                                    <span class="icon"><i class="fas fa-pen text-white" style="padding-top: 4px;"></i></span><span class="text">Edit</span>
                                                </a>
                                                <a href="javascript:;" data-toggle="modal" onclick="handleDelete({{$diagnosa->id}})" data-target="#DeleteModal" class="btn btn-sm btn-icon-split btn-danger">
