...
@section('content')
    @include('layouts.navbars.auth.topnav', ['title' => 'Perencanaan'])
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteModalLabel">Hapus Perencanaan</h5>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('importperencanaan') }}" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p class="text-danger">ANDA AKAN MENGUBAH STATUS ACC SEMUA PERENCANAAN ATAS NAMA PASIEN INI
                    </p>
                </div>
                <div class="modal-footer">
            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header d-sm-flex align-items-center justify-content-between py-3">
                        ...
                        @can('adminmahasiswa')
                            <a href="{{ route('perencanaan.create') }}"
                    ...
                    {{-- <div class="card-header d-sm-flex align-items-center">
                            @can('adminpembimbing')
                            <a href="{{route('exportperencanaan')}}" class="d-sm-inline-block btn btn-primary btn-sm shadow-sm me-3">
                                ...
                                <tbody>
                                    @foreach ($perencanaans as $perencanaan)
                                        <tr>
                                            ...
                                            @can('admin')
                                                <td>{{ $perencanaan->user->username }}</td>
                                                <td>{{ ucwords(get_v('users', 'nimnip', $perencanaan->user->pembimbing, 'username')[0]->username ?? '') }}
                                                </td>
                                            @endcan
                                            @can('mahasiswa')
                                                <td>{{ ucwords(get_v('users', 'nimnip', $perencanaan->user->pembimbing, 'username')[0]->username ?? '') }}
                                            ...
                                            @can('pembimbing')
                                                <td>{{ $perencanaan->user->username }}</td>
                                            @endcan
                                            <td>{{ $perencanaan->kartupasien->no_kartu }} |
                                                {{ $perencanaan->kartupasien->nama }}</td>
                                            {{-- pengetahuan --}}
                                            <td>{{ $perencanaan->gigi }} </td>
                                            <td>{{ $perencanaan->rasional }}</td>
                                            <td>{{ $perencanaan->kompetensi }}</td>
                                            <td>{{ $perencanaan->tujuan }}</td>
                                            <td>{{ $perencanaan->indikator }}</td>
                                            <td>{{ $perencanaan->cara_evaluasi }}</td>

                                            <td style="display: none;">
                                                @can('mahasiswa')
                                                    <label
                                                        class="{{ $perencanaan->acc == 0 ? 'btn btn-danger' : 'btn btn-success' }}">{{ $perencanaan->acc == 0 ? 'Belum' : 'Sudah' }}</label>
                                                @endcan
                                                @can('adminpembimbing')
                                                    @if ($perencanaan->acc == 0)
                                                        <a href="javascript:;" data-toggle="modal"
                                                            onclick="handleACC({{ $perencanaan->id }})" data-target="#AccModal"
                                            ...
                                            <td>
                                                @can('mahasiswa')
                                                    <label
                                                        class="{{ $perencanaan->acc == 0 ? 'btn btn-danger' : 'btn btn-success' }}">
                                                        {!! $perencanaan->acc == 0 ? '<i class="fas fa-times"></i>' : '<i class="fas fa-check"></i>' !!}
                                                    </label>
                                                @endcan
                                                @can('adminpembimbing')
                                                    @if ($perencanaan->acc == 0)
                                            ...
                                            <td>{{ date_format($perencanaan->created_at, 'd M Y') }}</td>
                                            <td>

                                                <a href ="{{ route('perencanaan.show', $perencanaan->id) }}" title="show"
                                                ...
                                                {{-- <a data-toggle="modal" id="smallButton" data-target="#smallModal"
                                                    data-attr="{{ route('anamriperencanaan.show', $project->id) }}" title="show">
                                                ...
                                                @can('adminmahasiswa')
                                                    @if ($perencanaan->acc !== 1)
                                                        ...
                                                        <a href="javascript:;" data-toggle="modal"
                                                            onclick="handleDelete({{ $perencanaan->id }})"
    ...
    @can('adminpembimbing')
        <script>
            function handleACC(id) {
                ...
                form.action = `./perencanaan/acc/${id}`
                console.log(form)
                $('#AccModal').modal('show')
            }
        </script>
